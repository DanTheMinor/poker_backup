...
module CompareCards
  hand_types = {
    ...
    "four of a kind" => [is_four_kind, compare_four_kind],
    "three of a kind" => [is_three_kind, compare_three_kind],
    ...
    def compare_cards(hand_1, hand_2)
      ...
      ["straight flush","four of a kind", "full house", "flush",
        "straight", "three of a kind", "two pair", "one pair", "high card"].each do |type|
    ...
    def compare_flush(other_hand)
      self.compare_high_card(other_hand)
    end
    ...
    def compare_four_kind(other_hand)
      self_values = self.map {|card| card.true_value}
      four_kind_value1 = self_values.select {|value| self_values.count(value) == 4}.first
      other_hand_values = other_hand.map {|card| card.true_value}
      four_kind_value2 = other_hand_values.select {|value| other_hand_values.count(value) == 4}.first

      if four_kind_value1 > four_kind_value2
        return self
      else
        return other_hand
      end
    ...
    def compare_two_pair(other_hand)
      self_values = self.map {|card| card.true_value}
      other_hand_values = other_hand.map {|card| card.true_value}
      self_paired_values = self_values.select {|value| self_values.count(value) == 2}.uniq.sort.reverse
      other_paired_values  = other_hand_values.select {|value| other_hand_values.count(value) == 2}.uniq.sort.reverse
      self_distinct_values = self_paired_values + self_values.select {|value| self_values.count(value) == 1}
      other_distinct_values = other_paired_values + other_hand_values.select {|value| other_hand_values.count(value) == 1}

      value_pairs = self_distinct_values.zip(other_distinct_values)
      value_pairs.each do |pair|
        if pair[0] > pair[1]
          return self
        elsif pair[1] > pair[0]
